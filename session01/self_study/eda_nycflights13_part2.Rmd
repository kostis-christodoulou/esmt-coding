...
```{r include=FALSE}
# This loads the necessary packages
library(mosaic)
library(lubridate)
...
How many flights are there to each airport in January, 2013? Both as a number and as a percentage of total flights for that month
...
ggplot(fly_into_sfo, aes(x=reorder(name, count), y=count, fill=name))+
  ...
  labs(y="Number of flights to SFO", x="Carrier", title="Flights to SFO", subtitle = "(as % of total flights)")+
    geom_label(aes(x=reorder(name, count), y=count, label = round(100*percent,1)),
             hjust = 1, 
             vjust = 0.5, 
             colour = "white", 
             fill = NA, 
...
```{r echo=TRUE, results="asis"}
weather |> 
  select(hour, dewp, humid, wind_speed, wind_dir, precip, pressure) |> 
  head()
```
